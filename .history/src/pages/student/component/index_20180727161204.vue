...
<!--     	<ul class="app-list">
		...
		<ul class="app-list" v-if="process.length>0">
    		<li v-for="(item, index) in process" @click="togo(item.pauId, item.state, item.studentJumpPath)" :key="index" :style="{'background-image':'url('+item.imgUrl+')','background-size': 'cover'}">
